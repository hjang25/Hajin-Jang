...
int write_ppm(FILE *fp, const Image *im) {
  size_t num_pix = 0;
  int size = (im->cols) * (im->rows); // for convenience

  // write header to the file
  fprintf(fp, "P6\n%d %d\n255\n", im->cols, im->rows);

  // write pixel values to the file
  num_pix = fwrite(im->data, sizeof(Pixel), size, fp);

  // handle errors
  if (!fp) { // file opening error
    return -1;
  }
  if (num_pix != (size_t)size) { // if not all elements were written
    return -1;
  }
  if (ferror(fp)) { // if error in writing file
    return -1;
  }

  // close file to prevent memory leakage
  fclose(fp);
...
void free_image(Image **im) {
  ...
  // two dynamically allocated variables: *im and (*im)->data
  // free and set inner pointer to NULL
  free((*im)->data);
  (*im)->data = NULL;

  // free and set outer pointer to NULL
  free(*im);
  *im = NULL;
}
...
int resize_image(Image **im, int rows, int cols) {
  // catch negative inputs for rows & cols
  if (rows < 0 || cols < 0) {
    return -1;
  }

  // reallocate image given rows & cols
  (*im)->data = realloc((*im)->data, (rows * cols) * sizeof(Pixel));

  // catch reallocation failure
  if ((*im)->data == NULL) {
    return -1;
  }
  return 0;
