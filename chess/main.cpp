	...
	while(!game_over) {
	  ...
	  if (choice.length() != 1) {
	    ...
	    case 'L': case 'l': {
			...
			// If game file doesn't exist, catch exception
			catch (const std::exception &exc) {
				// Return -1 and output exception to cerr 
	    ...
	    case 'M': case 'm': {
	      ...
	      if (argument.length() != 4) {
			std::cerr << "Move specifier must be four characters, but length(" << argument << " ) = " << 
			argument.length() << std::endl;
			// And make the move
	      } else {
			// TODO: add try catch blocks to recover from illegal moves
			try {
				// Trying if make_move passes without any exception
				game.make_move(std::make_pair(argument[0], argument[1]), std::make_pair(argument[2], argument[3])); 
			}
			catch (const std::exception &c) {
				// Catches all exceptions that ocur from make_move 
				// All different types of messages are output by c.what() 
				std::cerr << "Could not make move: " << c.what() << std::endl;
			}
	      }
	      break;
	    }
